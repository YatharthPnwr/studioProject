...
csv_file = os.path.join(eda_dir, "Electronics_cleaned.csv")
color = "#6366f1"  # Indigo
...
# Load first 200k rows (statistically robust downsampling to prevent memory limits)
df = pd.read_csv(csv_file, nrows=200000, escapechar='\\', low_memory=False)
...
print(f"📊 Dataset Size: {total_rows:,} products ({analyzed_rows:,} rows analyzed)")
...
print(f"\n--------------------------------------------------")
print(f"TOP BRANDS BY CATALGO VOLUME:")
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_rating_dist.png"), dpi=150, facecolor='#111827')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_price_dist.png"), dpi=150, facecolor='#111827')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_volume_vs_rating.png"), dpi=150, facecolor='#111827')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_correlation_heatmap.png"), dpi=150, facecolor='#111827')
...
if len(df_pivot) > 0:
    ...
    plt.title(f"Store vs. Category Rating Heatmap - {display_name}", fontsize=14, fontweight='bold', pad=15)
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_pivot_heatmap.png"), dpi=150, facecolor='#111827')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_temporal_line.png"), dpi=150, facecolor='#111827')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "Electronics_completeness.png"), dpi=150, facecolor='#111827')
